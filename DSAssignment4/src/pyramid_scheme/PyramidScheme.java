...
public class PyramidScheme extends LinkedTree<Person> {
    ...
    private ArrayList<MultiTreeNode<Person>> 
            findNodeChain(MultiTreeNode<Person> node, 
                    Person target) {
             java.util.Stack<MultiTreeNode<Person>>  stack =
                     new java.util.Stack<>(); 
             ...
             while (stack.isEmpty() == false) {
                 ...
                 MultiTreeNode<Person> curr = stack.pop();
                 if (curr.getElement().compareTo(target)==0) {
                     return chain; 
                 } else {
                     for(MultiTreeNode<Person> child : curr.getChildren()) {
                         stack.push(child);
                     }
                 chain.add(curr); 
                 }
