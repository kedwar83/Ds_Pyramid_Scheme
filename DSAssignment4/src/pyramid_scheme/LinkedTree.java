...
public class LinkedTree<T extends Comparable <T>> implements TreeADT<T> {
    ...
    public void addChild(T parent, T child) throws ElementNotFoundException {
        findNode(parent).addChild(new MultiTreeNode<T>(child)); 
    }
    ...
    private int countDown(MultiTreeNode<T> node) {
       if (node == null){
         ...
         return 1;   
           ...
           int count = 1; 
           ArrayList<MultiTreeNode<T>> children = node.getChildren();
           
           for (int i = 0; i < node.getChildren().size(); i++) {
               MultiTreeNode<T> child = node.getChildren().get(i);
               
           }
           
           
           for (MultiTreeNode<T> child : children) {
               count = count +countDown(child);
       }
           return count; 
       }
    }
